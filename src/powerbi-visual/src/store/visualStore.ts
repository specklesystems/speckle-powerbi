...
export const useVisualStore = defineStore('visualStore', () => { 
  ...
  const viewerEmit = ref<<E extends keyof IViewerEvents>(event: E,
    ...payload: Parameters<IViewerEvents[E]>) => void>()

  const dataInput = ref<SpeckleDataInput | null>()
  ...
  const setViewerEmitter = (emit: <E extends keyof IViewerEvents>(event: E,
    ...payload: Parameters<IViewerEvents[E]>) => void) => {
      if (emit) {
