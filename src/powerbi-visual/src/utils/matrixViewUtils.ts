...
async function fetchDataInBatches(id, batchSize = 10) {
  try {
    ...
    const rootResponse = await fetch(`http://localhost:29363/get-object/${id}`)
    ...
    for (const batch of batches) {
      ...
      const batchPromises = batch.map(async (childId) => {
        ...
        const response = await fetch(`http://localhost:29363/get-object/${childId}`)
...
async function fetchStreamedData(id) {
  try {
    const response = await fetch(`http://localhost:29363/get-objects/${id}`)
...
export async function processMatrixView(
  ...
  if (visualStore.lastLoadedRootObjectId !== id) {
    ...
    // try {
    //   const res = await fetch(`http://localhost:29363/get-data/${id}`)
